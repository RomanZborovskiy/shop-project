                ...
                <form method="GET" action="{{ route('client.shop.index') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="d-flex align-items-center">
                            <input type="number" name="price_from" class="form-control me-2" placeholder="Від" value="{{ request('price_from') }}">
                    ...
                    <a href="{{ route('client.shop.index') }}" class="btn btn-light w-100 mt-2">Скинути</a>
                </form>
            </div>
                            ...
                            <div class="product-item rounded wow fadeInUp" data-wow-delay="0.1s">
                                <div class="product-item-inner border rounded">
                                    <div class="product-item-inner-item">
                                        <img src="{{ $product->getFirstMediaUrl('images', 'thumb') }}" class="img-fluid w-100 rounded-top" alt="{{ $product->name }}">
                                        <div class="product-details">
                                            <a href="{{ route('client.shop.show', $product) }}"><i class="fa fa-eye fa-1x"></i></a>
                                        </div>
                                    </div>
                                    <div class="text-center rounded-bottom p-4">
                                        <a href="{{ route('client.shop.index', ['brand_id' => $product->brand->id]) }}" class="d-block mb-2">{{ $product->brand->name ?? '' }}</a>
                                        <a href="{{ route('client.shop.show', $product) }}" class="d-block h4">{{ $product->name }}</a>
